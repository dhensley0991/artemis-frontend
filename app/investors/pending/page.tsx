...
export default function PendingInvestorsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const safeFunds = Array.isArray(fundsData) ? fundsData : [];

            setPendingInvestors(safePending);
            setFunds(safeFunds);

            const initialSelectedFunds: Record<number, string> = {};
            safePending.forEach((inv) => {
                if (inv.requested_fund_id) {
                    initialSelectedFunds[inv.id] = String(inv.requested_fund_id);
                }
            });

            setSelectedFundByInvestor(initialSelectedFunds);
        } catch (err) {
