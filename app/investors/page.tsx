...
export default function InvestorsPage() {
  ...
  useEffect(() => {
    ...
    const loadInvestorReport = async () => {
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/investor-report`, {
          headers: {
        ...
        setError(err instanceof Error ? err.message : "Failed to load investor report");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (fundFilter !== "all") {
      next = next.filter((row) => (row.fund_name || "Unassigned") === fundFilter);
    }

    if (shareClassFilter !== "all") {
      next = next.filter((row) => (row.share_class || "Unassigned") === shareClassFilter);
    }
  ...
  const fundOptions = useMemo(() => {
    const values = Array.from(new Set(rows.map((row) => row.fund_name || "Unassigned")));
    return values.sort((a, b) => a.localeCompare(b));
  }, [rows]);

  const shareClassOptions = useMemo(() => {
    const values = Array.from(new Set(rows.map((row) => row.share_class || "Unassigned")));
    return values.sort((a, b) => a.localeCompare(b));
  }, [rows]);

  const metrics = useMemo(() => {
    const totalInvestors = filteredRows.length;
    const totalSubscribed = filteredRows.reduce((sum, row) => sum + (row.total_subscribed || 0), 0);
    const totalCurrentValue = filteredRows.reduce((sum, row) => sum + (row.current_value || 0), 0);
    const totalFees = filteredRows.reduce((sum, row) => sum + (row.fees_paid || 0), 0);
    const totalUnits = filteredRows.reduce((sum, row) => sum + (row.units_owned || 0), 0);
    const avgPerformance =
      totalInvestors > 0
        ? filteredRows.reduce((sum, row) => sum + (row.performance_percent || 0), 0) / totalInvestors
        : 0;
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-slate-400">
                  Investor reporting across funds, share classes, ownership, capital balances, fees,
                  and performance in one institutional view.
                </p>
              </div>
            </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Total Investors</p>
            <p className="mt-3 text-3xl font-semibold text-[#F1D36B]">{metrics.totalInvestors}</p>
          </div>

          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Capital Invested</p>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Units Owned</p>
            <p className="mt-3 text-3xl font-semibold text-[#F1D36B]">
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Fees Paid</p>
            <p className="mt-3 text-3xl font-semibold text-[#F1D36B]">
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Avg Performance</p>
            <p className="mt-3 text-3xl font-semibold text-[#F1D36B]">
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl">
          <div className="mb-4 flex items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold text-[#D4AF37]">Investor Report</h2>
          ...
          {loading ? (
            ...
            <p className="text-sm text-slate-400">No investor records found yet.</p>
          ) : (
            <div className="overflow-x-auto rounded-2xl border border-white/10">
              <table className="min-w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-white/[0.03] text-left">
                    <th className="px-4 py-3 font-medium text-slate-300">Investor</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Account #</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Fund</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Fund ID</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Share Class</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Investor Since</th>
                    ...
                    <th className="px-4 py-3 font-medium text-slate-300">Subscribed</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Opening</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Closing</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Current Value</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Units</th>
                    <th className="px-4 py-3 font-medium text-slate-300">Fees</th>
                    <th className="px-4 py-3 font-medium text-slate-300">P/L ($)</th>
                    <th className="px-4 py-3 font-medium text-slate-300">P/L (%)</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <tr key={row.account_id} className="border-b border-white/5 transition hover:bg-white/[0.04]">
                        <td className="px-4 py-3 text-white">{row.investor_name || "-"}</td>
                        <td className="px-4 py-3 text-slate-300">{row.investor_account_number || "-"}</td>
                        <td className="px-4 py-3 text-white">{row.fund_name || "-"}</td>
                        <td className="px-4 py-3 text-[#F1D36B]">{row.fund_id || "-"}</td>
                        <td className="px-4 py-3 text-slate-300">{row.share_class || "-"}</td>
                        <td className="px-4 py-3 text-slate-300">{formatDate(row.investor_since)}</td>
                        <td className="px-4 py-3 text-slate-300">{row.investor_email || "-"}</td>
                        <td className="px-4 py-3 text-slate-300">{row.investor_phone || "-"}</td>
                        <td className="px-4 py-3 text-slate-300">{buildAddress(row.investor_address)}</td>
                        <td className="px-4 py-3 text-[#F1D36B]">{formatMoney(row.total_subscribed)}</td>
                        <td className="px-4 py-3 text-slate-300">{formatMoney(row.opening_balance)}</td>
                        <td className="px-4 py-3 text-slate-300">{formatMoney(row.closing_balance)}</td>
                        <td className="px-4 py-3 text-[#F1D36B]">{formatMoney(row.current_value)}</td>
                        ...
                        <td className="px-4 py-3 text-slate-300">{formatMoney(row.fees_paid)}</td>
