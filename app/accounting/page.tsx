...
export default function AccountingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/ignisratio/entries`,{headers:{Authorization:`Bearer ${token}`}}),fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/ignisratio/files`,{headers:{Authorization:`Bearer ${token}`}})])
      .then(async ([entryResponse,fileResponse])=>{const entryData=await entryResponse.json();if(!entryResponse.ok)throw new Error(entryData.detail||"Unable to load accounting records");const fileData=await fileResponse.json();if(!fileResponse.ok)throw new Error(fileData.detail||"Unable to load invoice files");return {entryData:entryData as Entry[],fileData:fileData as StoredFile[]};})
      .then(data=>{setEntries(data.entryData);setFiles(data.fileData);setReady(true);})
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(212,175,55,0.08),_transparent_30%),linear-gradient(to_bottom,_#000,_#0b0b0b,_#171717)] text-white">
    <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-8">
      ...
      {active==="Files"&&<FilesPanel files={files} entries={entries} onOpen={file=>openStoredFile(file)} onDownload={file=>openStoredFile(file,true)} onDelete={deleteStoredFile}/>}
