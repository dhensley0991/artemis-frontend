...
export default function AccountingPage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(212,175,55,0.08),_transparent_30%),linear-gradient(to_bottom,_#000,_#0b0b0b,_#171717)] text-white">
    <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-8">
      ...
      {["Transactions","Receivables","Payables","Contractors","Shareholder"].includes(active)&&<div className="mt-5"><LedgerTable title={active} entries={filtered} onEdit={entry=>{setEditing(entry);setOpen(true)}} onDelete={deleteEntry}/></div>}
      {active==="Tax Center"&&<TaxCenter profit={totals.profit}/>} {active==="Reports"&&<Reports totals={totals} onExport={exportCsv}/>} 
    </div>
    {open&&<EntryModal entry={editing} onClose={()=>{setOpen(false);setEditing(null)}} onSubmit={saveEntry}/>}
  </main>;
}
...
function EntryModal({entry,onClose,onSubmit}:{entry:Entry|null;onClose:()=>void;onSubmit:(e:FormEvent<HTMLFormElement>)=>void}){return <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><form onSubmit={onSubmit} className="w-full max-w-2xl rounded-[26px] border border-white/15 bg-[#151515] p-6 shadow-2xl"><div className="flex justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-slate-500">IgnisRatio · Ridge Four Holdings, LLC</p><h2 className="mt-1 text-2xl font-semibold text-[#F1D36B]">{entry?"Edit record":"Add record"}</h2></div><button type="button" onClick={onClose} className="text-2xl text-slate-400">×</button></div><div className="mt-6 grid gap-4 md:grid-cols-2"><Field label="Type"><select name="type" className="field" defaultValue={entry?.type||"income"}><option value="income">Income received</option><option value="expense">Expense</option><option value="invoice">Customer invoice</option><option value="bill">Vendor bill</option><option value="contractor">1099 contractor payment</option><option value="shareholder">Shareholder activity</option></select></Field><Field label="Status"><select name="status" className="field" defaultValue={entry?.status||"paid"}><option value="paid">Paid</option><option value="open">Open</option><option value="received">Received</option><option value="w9-needed">W-9 needed</option><option value="posted">Posted</option></select></Field><Field label="Name"><input name="name" required className="field" placeholder="Customer, vendor, or description" defaultValue={entry?.name||""}/></Field><Field label="Amount"><input name="amount" required min="0" step="0.01" type="number" className="field" placeholder="0.00" defaultValue={entry?.amount}/></Field><Field label="Category"><input name="category" required className="field" placeholder="Professional services" defaultValue={entry?.category||""}/></Field><Field label="Date"><input name="date" required type="date" className="field" defaultValue={entry?.date||new Date().toISOString().slice(0,10)}/></Field><div className="md:col-span-2"><Field label="Memo"><textarea name="memo" className="field min-h-20" placeholder="Business purpose or supporting note" defaultValue={entry?.memo||""}/></Field></div></div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl border border-white/15 px-4 py-2">Cancel</button><button className="rounded-xl bg-[#D4AF37] px-5 py-2 font-semibold text-black">{entry?"Update record":"Save record"}</button></div></form></div>}
